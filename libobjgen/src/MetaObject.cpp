...
MetaObject::MetaObject()
    : mNamespace("objects"),
      mScriptEnabled(false),
      mPersistent(false),
      mInheritedConstruction(false) {}

MetaObject::~MetaObject() {}

std::string MetaObject::GetName() const { return mName; }

std::string MetaObject::GetNamespace() const { return mNamespace; }

std::string MetaObject::GetBaseObject() const { return mBaseObject; }

bool MetaObject::IsPersistent() const { return mPersistent; }
...
bool MetaObject::IsScriptEnabled() const { return mScriptEnabled; }

void MetaObject::SetScriptEnabled(bool scriptEnabled) {
  mScriptEnabled = scriptEnabled;
}

std::string MetaObject::GetSourceLocation() const { return mSourceLocation; }
...
bool MetaObject::AddVariable(const std::shared_ptr<MetaVariable>& var) {
  ...
  if (IsValidIdentifier(name) &&
      mVariableMapping.end() == mVariableMapping.find(name)) {
...
bool MetaObject::RemoveVariable(const std::string& name) {
  ...
  if (mVariableMapping.end() != entry) {
    auto orderedEntry =
        std::find(mVariables.begin(), mVariables.end(), entry->second);
...
bool MetaObject::Load(std::istream& stream) {
  ...
  stream.read(reinterpret_cast<char*>(&mScriptEnabled), sizeof(mScriptEnabled));
  stream.read(reinterpret_cast<char*>(&mPersistent), sizeof(mPersistent));
...
std::set<std::string> MetaObject::GetReferencesTypes(
...
void MetaObject::AddInheritedObject(const std::shared_ptr<MetaObject>& obj) {
  if (mInheritedObjects.end() ==
      std::find(mInheritedObjects.begin(), mInheritedObjects.end(), obj)) {
...
void MetaObject::GetAllInheritedObjects(
  ...
  objs.insert(objs.end(), mInheritedObjects.begin(), mInheritedObjects.end());
