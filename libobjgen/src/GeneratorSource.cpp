...
std::string GeneratorSource::Generate(const MetaObject& obj) {
  ...
  for (auto var : obj.GetReferences()) {
    references.push_back(std::dynamic_pointer_cast<MetaVariableReference>(var));
  }
  ...
  if (!references.empty()) {
    ...
    for (auto ref : references) {
      ss << "#include <"
         << Generator::GetObjectName(ref->GetReferenceType(true)) << ".h>"
         << std::endl;
    }
  ...
  for (auto it = obj.VariablesBegin(); it != obj.VariablesEnd(); ++it) {
    ...
    std::string constructor =
        var->GetConstructorCode(*this, obj, GetMemberName(var));
  ...
  for (auto it = obj.VariablesBegin(); it != obj.VariablesEnd(); ++it) {
    ...
    std::string destructor =
        var->GetDestructorCode(*this, obj, GetMemberName(var));
  ...
  ss << Tab()
     << "bool status = " +
            GetBaseBooleanReturnValue(obj, "IsValid(recursive)") + ";"
     << std::endl;
  ...
  for (auto it = obj.VariablesBegin(); it != obj.VariablesEnd(); ++it) {
    ...
    std::string validator =
        var->GetValidCondition(*this, GetMemberName(var), true);
  ...
  // Load (binary)
  ss << "bool " << obj.GetName() << "::Load(libcomp::ObjectInStream& stream)"
     << std::endl;
  ss << "{" << std::endl;
  ss << Tab()
     << "bool status = " + GetBaseBooleanReturnValue(obj, "Load(stream)") + ";"
     << std::endl;
  ...
  for (auto it = obj.VariablesBegin(); it != obj.VariablesEnd(); ++it) {
    ...
    std::string code = var->GetLoadCode(*this, GetMemberName(var), "stream");
  ...
  ss << std::endl;
  ss << Tab()
     << "bool status = " + GetBaseBooleanReturnValue(obj, "Save(stream)") + "; "
     << std::endl;
  ...
  for (auto it = obj.VariablesBegin(); it != obj.VariablesEnd(); ++it) {
    ...
    std::string code = var->GetSaveCode(*this, GetMemberName(var), "stream");
  ...
  // Load (raw binary)
  ss << "bool " << obj.GetName() << "::Load(std::istream& stream, bool flat)"
     << std::endl;
  ss << "{" << std::endl;
  ...
  ss << Tab()
     << "bool status = " +
            GetBaseBooleanReturnValue(obj, "Load(stream, flat)") + ";"
     << std::endl;
  ...
  for (auto it = obj.VariablesBegin(); it != obj.VariablesEnd(); ++it) {
    ...
    std::string code = var->GetLoadRawCode(*this, GetMemberName(var), "stream");
  ...
  ss << Tab()
     << "bool status = " +
            GetBaseBooleanReturnValue(obj, "Save(stream, flat)") + "; "
     << std::endl;
  ...
  for (auto it = obj.VariablesBegin(); it != obj.VariablesEnd(); ++it) {
    ...
    std::string code = var->GetSaveRawCode(*this, GetMemberName(var), "stream");
  ...
  // Load (XML)
  ss << "bool " << obj.GetName() << "::Load(const tinyxml2::XMLDocument& doc, "
     << std::endl;
  ...
  ss << std::endl;
  ss << Tab()
     << "bool status = " + GetBaseBooleanReturnValue(obj, "Load(doc, root)") +
            ";"
     << std::endl;
  ss << std::endl;
  ...
  for (auto it = obj.VariablesBegin(); it != obj.VariablesEnd(); ++it) {
    ...
    std::string code =
        var->GetXmlLoadCode(*this, GetMemberName(var), "doc", "pMember");
    ...
    if (!code.empty()) {
      ...
      ss << Tab(2) << "{" << std::endl;
      ss << Tab(3)
         << "LogGeneralErrorMsg(libcomp::String(\"Failed to load member '"
  ...
  // Save (XML)
  ss << "bool " << obj.GetName() << "::Save(tinyxml2::XMLDocument& doc, "
     << std::endl;
  ...
  if (!baseObject.empty()) {
    ss << std::endl
       << Tab() << "status &= " << baseObject << "::Save(doc, *pElement, true);"
       << std::endl;
  }
  ...
  for (auto it = obj.VariablesBegin(); it != obj.VariablesEnd(); ++it) {
    ...
    std::string code =
        var->GetXmlSaveCode(*this, GetMemberName(var), "doc", "pElement");
  ...
  ss << "uint16_t " << obj.GetName() << "::GetDynamicSizeCount() const"
     << std::endl;
  ss << "{" << std::endl;
  ...
  for (auto it = obj.VariablesBegin(); it != obj.VariablesEnd(); ++it) {
    ...
    if (scriptEnabled && var->IsScriptAccessible()) {
      auto binding =
          var->GetAccessScriptBindings(*this, obj, GetMemberName(var));
      if (binding.length() > 0) {
  ...
  if (scriptEnabled) {
    ...
    if (!baseObject.empty()) {
      bindingType << "DerivedClass<" << obj.GetName() << ", " << baseObject
                  << ">";
      ...
      parent_dependency << Tab(3) << "Using<libcomp::PersistentObject>();"
                        << std::endl;
      dependency_prototypes << Tab() << "template<>" << std::endl;
      dependency_prototypes
          << Tab()
    ...
    for (auto it = obj.VariablesBegin(); it != obj.VariablesEnd(); ++it) {
      ...
      if (var->GetMetaType() == MetaVariable::MetaVariableType_t::TYPE_ENUM) {
        ...
        additions << std::endl
                  << Tab() << "Sqrat::ConstTable(mVM).Enum(\"" << obj.GetName()
                  << "_" << eVar->GetName() << "_t\", e);" << std::endl
                  << "}" << std::endl
                  << std::endl;
      }
    }
    ...
    if (obj.IsPersistent()) {
      ...
      additions << ParseTemplate(0, "VariablePersistentReferenceScriptBinding",
                                 replacements);
    }
...
bool GeneratorSource::GeneratePersistentObjectFunctions(const MetaObject& obj,
  ...
  for (auto it = obj.VariablesBegin(); it != obj.VariablesEnd(); ++it) {
    ...
    binds << Tab() << "{" << std::endl;
    binds << Tab(1) << "values.push_back(("
          << var->GetBindValueCode(*this, GetMemberName(var)) << ")());"
          << std::endl;
    binds << Tab() << "}" << std::endl;
    binds << std::endl;
  }
  ...
  for (auto it = obj.VariablesBegin(); it != obj.VariablesEnd(); ++it) {
    ...
    dbValues << Tab() << "if(!"
             << var->GetDatabaseLoadCode(*this, GetMemberName(var)) << ")"
             << std::endl;
...
std::string GeneratorSource::GetBaseBooleanReturnValue(
    const MetaObject& obj, std::string function, std::string defaultValue) {
