...
MetaVariableEnum::MetaVariableEnum()
    : MetaVariable(), mUnderlyingType("int32_t") {}

MetaVariableEnum::~MetaVariableEnum() {}

std::string MetaVariableEnum::GetDefaultValue() const { return mDefaultValue; }

void MetaVariableEnum::SetDefaultValue(const std::string& value) {
  mDefaultValue = value;
}

std::string MetaVariableEnum::GetTypePrefix() const { return mTypePrefix; }
...
bool MetaVariableEnum::SetValues(
...
std::string MetaVariableEnum::GetType() const { return "enum"; }

bool MetaVariableEnum::IsCoreType() const { return false; }
...
std::string MetaVariableEnum::GetValidCondition(const Generator& generator,
  ...
  if (mValues.size() > 0) {
    ss << name << " >= " << GetCodeType() << "::" << mValues[0].first << " && "
       << name << " <= " << GetCodeType()
       << "::" << mValues[mValues.size() - 1].first;
  }
...
std::string MetaVariableEnum::GetLoadRawCode(const Generator& generator,
  ...
  code = generator.ParseTemplate(0, "VariableEnumLoad", replacements);
...
std::string MetaVariableEnum::GetSaveRawCode(const Generator& generator,
  ...
  code = generator.ParseTemplate(0, "VariableEnumSave", replacements);
...
std::string MetaVariableEnum::GetXmlLoadCode(const Generator& generator,
  ...
  return generator.ParseTemplate(tabLevel, "VariableEnumXmlLoad", replacements);
}
...
std::string MetaVariableEnum::GetXmlSaveCode(const Generator& generator,
  ...
  return generator.ParseTemplate(tabLevel, "VariableEnumXmlSave", replacements);
}
...
std::string MetaVariableEnum::GetAccessDeclarations(const Generator& generator,
  ...
  ss << generator.ParseTemplate(tabLevel, "VariableEnumAccessDeclarations",
                                replacements)
     << std::endl;
...
std::string MetaVariableEnum::GetAccessFunctions(
  ...
  ss << std::endl
     << generator.ParseTemplate(0, "VariableEnumAccessFunctions", replacements)
     << std::endl;
...
std::string MetaVariableEnum::GetUtilityDeclarations(const Generator& generator,
  ...
  ss << generator.ParseTemplate(tabLevel, "VariableEnumUtilityDeclarations",
                                replacements)
     << std::endl;
...
std::string MetaVariableEnum::GetUtilityFunctions(
    const Generator& generator, const MetaObject& object,
    const std::string& name) const {
  (void)name;
  ...
  ss << std::endl
     << generator.ParseTemplate(0, "VariableEnumUtilityFunctions", replacements)
     << std::endl;

  return ss.str();
}

std::string MetaVariableEnum::GetAccessScriptBindings(
    const Generator& generator, const MetaObject& object,
  ...
  ss << generator.ParseTemplate(tabLevel, "VariableEnumAccessScriptBindings",
                                replacements)
     << std::endl;
...
bool MetaVariableEnum::ContainsDuplicateValues(
  ...
  for (size_t i = 0; i < values.size(); i++) {
    for (size_t k = i + 1; k < values.size(); k++) {
      if (values[i].first == values[k].first ||
          (values[i].second == values[k].second && !values[i].second.empty())) {
        return true;
      }
    }
  }
