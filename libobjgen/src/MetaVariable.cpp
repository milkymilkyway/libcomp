...
std::string MetaVariable::GetName() const { return mName; }

void MetaVariable::SetName(const std::string& name) { mName = name; }

bool MetaVariable::IsInherited() const { return mInherited; }

void MetaVariable::SetInherited(const bool inherited) {
  mInherited = inherited;
}

bool MetaVariable::IsLookupKey() const { return mLookupKey; }
...
bool MetaVariable::IsUniqueKey() const { return mUniqueKey; }
...
unsigned char MetaVariable::GetPadding() const { return mPadding; }
...
std::string MetaVariable::GetError() const { return mError; }

bool MetaVariable::IsCaps() const { return mCaps; }

void MetaVariable::SetCaps(bool caps) { mCaps = caps; }

uint16_t MetaVariable::GetDynamicSizeCount() const { return 0; }

bool MetaVariable::Load(std::istream& stream) {
  Generator::LoadString(stream, mName);
  stream.read(reinterpret_cast<char*>(&mCaps), sizeof(mCaps));
  stream.read(reinterpret_cast<char*>(&mInherited), sizeof(mInherited));
  stream.read(reinterpret_cast<char*>(&mLookupKey), sizeof(mLookupKey));
  stream.read(reinterpret_cast<char*>(&mUniqueKey), sizeof(mUniqueKey));
  stream.read(reinterpret_cast<char*>(&mPadding), sizeof(mPadding));
...
bool MetaVariable::Save(std::ostream& stream) const {
  ...
  if (IsValid()) {
    ...
    stream.write(reinterpret_cast<const char*>(&mCaps), sizeof(mCaps));
    ...
    stream.write(reinterpret_cast<const char*>(&mUniqueKey),
                 sizeof(mUniqueKey));
    stream.write(reinterpret_cast<const char*>(&mPadding), sizeof(mPadding));
...
bool MetaVariable::LoadVariableList(
    std::istream& stream, std::list<std::shared_ptr<MetaVariable>>& vars) {
  size_t varCount;
  stream.read(reinterpret_cast<char*>(&varCount), sizeof(varCount));

  if (stream.good()) {
    for (size_t i = 0; i < varCount; i++) {
      MetaVariableType_t metaType;
      stream.read(reinterpret_cast<char*>(&metaType), sizeof(metaType));

      short subTypeCount;
      stream.read(reinterpret_cast<char*>(&subTypeCount), sizeof(subTypeCount));
      ...
      for (short k = 0; k < subTypeCount; k++) {
        ...
        stream.read(reinterpret_cast<char*>(&subMetaType), sizeof(subMetaType));
...
bool MetaVariable::SaveVariableList(
    std::ostream& stream,
    const std::list<std::shared_ptr<MetaVariable>>& vars) {
  size_t varCount = vars.size();
  stream.write(reinterpret_cast<const char*>(&varCount), sizeof(varCount));

  if (stream.good()) {
    for (auto var : vars) {
      auto metaType = var->GetMetaType();
      stream.write(reinterpret_cast<const char*>(&metaType), sizeof(metaType));

      std::list<std::shared_ptr<MetaVariable>> subTypes;
      switch (metaType) {
        case MetaVariableType_t::TYPE_ARRAY: {
          auto arr = std::dynamic_pointer_cast<MetaVariableArray>(var);

          subTypes.push_back(arr->GetElementType());
        } break;
        case MetaVariableType_t::TYPE_LIST: {
          auto l = std::dynamic_pointer_cast<MetaVariableList>(var);

          subTypes.push_back(l->GetElementType());
        } break;
        case MetaVariableType_t::TYPE_SET: {
          auto l = std::dynamic_pointer_cast<MetaVariableSet>(var);

          subTypes.push_back(l->GetElementType());
        } break;
        case MetaVariableType_t::TYPE_MAP: {
          auto m = std::dynamic_pointer_cast<MetaVariableMap>(var);
...
std::shared_ptr<MetaVariable> MetaVariable::CreateType(
  ...
  if (std::regex_match(typeName, match, re)) {
    ...
    if (!var || !std::dynamic_pointer_cast<MetaVariableReference>(var)
                     ->SetReferenceType(match[1])) {
...
std::shared_ptr<MetaVariable> MetaVariable::CreateType(
    const MetaVariableType_t type, std::vector<MetaVariableType_t> subtypes) {
  switch (type) {
...
std::string MetaVariable::GetBindValueCode(const Generator& generator,
  ...
  return generator.ParseTemplate(tabLevel, "VariableGetBind", replacements);
}
...
std::string MetaVariable::GetSetterCode(const Generator& generator,
  ...
  std::string persistentCode =
      object.IsPersistent() ? ("mDirtyFields.insert(\"" + GetName() + "\");")
                            : "";
  if (condition.empty()) {
    ss << generator.Tab(tabLevel) << name << " = " << argument << ";"
       << std::endl;
    ...
    ss << generator.Tab(tabLevel + 1) << name << " = " << argument << ";"
       << std::endl;
...
std::string MetaVariable::GetAccessDeclarations(const Generator& generator,
  ...
  ss << generator.Tab(tabLevel) << "bool Set" << generator.GetCapitalName(*this)
...
std::string MetaVariable::GetAccessFunctions(const Generator& generator,
  ...
  ss << GetCodeType() << " " << objName << "::"
     << "Get" << generator.GetCapitalName(*this) << "() const" << std::endl;
  ss << "{" << std::endl;
...
std::string MetaVariable::GetAccessScriptBindings(const Generator& generator,
  ...
  auto objName = object.GetName();
  ss << ".Func(\"Get" << generator.GetCapitalName(*this) << "\", &" << objName
     << "::Get" << generator.GetCapitalName(*this) << ")" << std::endl;

  ss << ".Func(\"Set" << generator.GetCapitalName(*this) << "\", &" << objName
     << "::Set" << generator.GetCapitalName(*this) << ")" << std::endl;

  ss << ".Prop("
     << "\"" << generator.GetCapitalName(*this) << "\", &" << objName << "::Get"
     << generator.GetCapitalName(*this) << ", &" << objName << "::Set"
  ...
  if (IsLookupKey()) {
    ...
    ss << ".StaticFunc(\"" << f.str() << "By" << generator.GetCapitalName(*this)
       << "\", " << objName << "::" << f.str() << "By"
       << generator.GetCapitalName(*this) << ")" << std::endl;
  }
...
std::string MetaVariable::GetConstructorCode(const Generator& generator,
  ...
  if (code.empty()) {
    ...
    std::stringstream ss;
    if (IsInherited()) {
      ss << generator.Tab(tabLevel) << "Set" << generator.GetCapitalName(*this)
         << "(" << code << ")"
         << ";" << std::endl;
...
std::string MetaVariable::GetDynamicSizeCountCode(
    const Generator& generator, const std::string& name) const {
  (void)generator;
  (void)name;
