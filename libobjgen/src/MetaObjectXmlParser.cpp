...
std::string MetaObjectXmlParser::GetError() const { return mError; }
...
bool MetaObjectXmlParser::LoadTypeInformation(
  ...
  if (nullptr != szTagName && "object" == std::string(szTagName)) {
    ...
    const char *szInheritedConstruction =
        root.Attribute("inherited-construction");
    ...
    if (nullptr != szName && mObject->SetName(szName)) {
      ...
      if (nullptr != szPersistent) {
        mObject->mPersistent = Generator::GetXmlAttributeBoolean(szPersistent);
      } else {
      ...
      if (nullptr != szInheritedConstruction) {
        mObject->mInheritedConstruction =
            Generator::GetXmlAttributeBoolean(szInheritedConstruction);
      }
      ...
      if (nullptr != szScriptEnabled) {
        mObject->mScriptEnabled =
            Generator::GetXmlAttributeBoolean(szScriptEnabled);
      } else {
...
bool MetaObjectXmlParser::LoadMembers(const std::string &object,
  ...
  if (nullptr == mObject) {
    std::stringstream ss;
    ss << "Unkown object '" << object << "' could not have its members loaded.";
  ...
  if (mMemberLoadedObjects.find(object) != mMemberLoadedObjects.end()) {
    std::stringstream ss;
    ss << "Object '" << object << "' has already had its members loaded.";
...
bool MetaObjectXmlParser::FinalizeObjectAndReferences(
  ...
  if (nullptr == mObject) {
    std::stringstream ss;
    ss << "Unkown object '" << object << "' could not be finalized.";
  ...
  while (requiresLoad.size() > 0) {
    ...
    if (mObjectXml.find(objectName) == mObjectXml.end()) {
      std::stringstream ss;
      ss << "Object '" << objectName << "' is not defined.";
    ...
    if (mMemberLoadedObjects.find(objectName) == mMemberLoadedObjects.end()) {
      ...
      if (err != tinyxml2::XML_SUCCESS) {
        std::stringstream ss;
        ss << "Object '" << objectName << "' XML parsing failed.";
    ...
    for (auto var : obj->GetReferences()) {
      auto ref =
          std::dynamic_pointer_cast<libobjgen::MetaVariableReference>(var);
  ...
  if (HasCircularReference(mObject, std::set<std::string>())) {
    std::stringstream ss;
    ss << "Object contains circular reference: " << object;
  ...
  if (mObject->IsScriptEnabled()) {
    ...
    if (!baseObject.empty() && !GetKnownObject(baseObject)->IsScriptEnabled()) {
      std::stringstream ss;
      ss << "Script enabled object is derived from an object that is not "
            "script enabled: "
         << object;
...
bool MetaObjectXmlParser::LoadMember(const tinyxml2::XMLDocument &doc,
  ...
  if (nullptr != szMemberName && mObject->IsValidIdentifier(szMemberName)) {
    std::shared_ptr<MetaVariable> var =
        GetVariable(doc, szName, szMemberName, pMember);
    if (var) {
      var->SetName(szMemberName);

      if (var->IsLookupKey() && !mObject->IsPersistent()) {
        std::stringstream ss;
        ss << "Non-persistent object member variable '" << szMemberName
           << "' on object '" << szName << "' marked as a lookup key.";
        ...
        std::stringstream ss;
        ss << "Failed to add member variable '" << szMemberName
           << "' to object '" << szName
           << "'. A variable by the same name may already exist.";
...
std::shared_ptr<MetaVariable> MetaObjectXmlParser::GetVariable(
  ...
  if (nullptr == szMemberType) {
    std::stringstream ss;
    ss << "Member variable '" << szMemberName << "' for object '" << szName
    ...
    if (!var && (memberType == "list" || memberType == "array" ||
                 memberType == "set" || memberType == "map")) {
      ...
      if (memberType == "list") {
        if (nullptr != subElems["element"]) {
          var = std::shared_ptr<MetaVariable>(
              new MetaVariableList(subElems["element"]));
        } else {
          std::stringstream ss;
          ss << "Failed to parse list member '" << szMemberName
             << "' element in object '" << szName << "'";
        ...
        if (nullptr != subElems["element"]) {
          var = std::shared_ptr<MetaVariable>(
              new MetaVariableArray(subElems["element"]));
        } else {
          std::stringstream ss;
        ...
        if (nullptr != subElems["element"]) {
          var = std::shared_ptr<MetaVariable>(
              new MetaVariableSet(subElems["element"]));
        } else {
          std::stringstream ss;
        ...
        if (nullptr != subElems["key"] && nullptr != subElems["value"]) {
          ...
          if (keyMetaType == MetaVariable::MetaVariableType_t::TYPE_ARRAY ||
            ...
            ss << "Invalid map key type of '" << key->GetType()
               << "' specified for member '" << szMemberName << "' on object '"
               << szName << "'";

            mError = ss.str();
          } else if (valueMetaType ==
                         MetaVariable::MetaVariableType_t::TYPE_ARRAY ||
                     valueMetaType ==
                         MetaVariable::MetaVariableType_t::TYPE_LIST ||
                     valueMetaType ==
                         MetaVariable::MetaVariableType_t::TYPE_SET ||
                     valueMetaType ==
                         MetaVariable::MetaVariableType_t::TYPE_MAP) {
            std::stringstream ss;
            ...
            if (valueMetaType == MetaVariable::MetaVariableType_t::TYPE_ENUM) {
              ...
              if (seMember && !enumName.empty()) {
                ...
                ss << "Failed to parse map member '" << szMemberName
                   << "' because the value is an enumeration without a 'name' "
                      "attribute";
          ...
          std::stringstream ss;
          ss << "Failed to parse map member '" << szMemberName
             << "' key and value on object '" << szName << "'";

          mError = ss.str();
        }
      }
    } else if (var && var->GetMetaType() ==
                          MetaVariable::MetaVariableType_t::TYPE_ENUM) {
      std::dynamic_pointer_cast<MetaVariableEnum>(var)->SetTypePrefix(
          mObject->GetName());
    } else if (var && var->GetMetaType() ==
                          MetaVariable::MetaVariableType_t::TYPE_REF) {
      ...
      if (genericReference) {
        ...
        if (mKnownObjects.find(refType) == mKnownObjects.end()) {
          std::stringstream ss;
          ss << "Unknown reference type '" << refType
             << "' encountered on field '" << szMemberName << "' in object '"
             << szName << "'.";

          mError = ss.str();
        } else if (mKnownObjects[refType]->GetNamespace() !=
                   ref->GetNamespace()) {
          std::stringstream ss;
          ...
          if (!persistentRefType && mKnownObjects[szName]->IsPersistent()) {
            std::stringstream ss;
            ss << "Non-peristent reference type '" << refType
               << "' encountered on field '" << szMemberName
               << "' in persistent object '" << szName << "'.";
        ...
        while (nullptr != cMember) {
          ...
          if ("member" == cMemberName) {
            ...
            if (persistentRefType && cName != "UID") {
              std::stringstream ss;
              ss << "Persistent reference type '" << refType << "' on field '"
                 << szMemberName << "' in object '" << szName
              ...
              std::stringstream ss;
              ss << "Non-persistent generic reference type on field '"
                 << szMemberName << "' in object '" << szName
    ...
    if (nullptr != szMemberPadding) {
      ...
      if (ss && 0 < elemPadding) {
        ...
        ss.clear();
        ss << "Failed to parse member 'pad' in object '" << szName
           << "': " << var->GetError();
    ...
    if (var && var->Load(doc, *pMember)) {
      ...
      std::stringstream ss;
      ss << "Failed to parse member '" << szMemberName << "' of type '"
         << szMemberType << "' in object '" << szName
         << "': " << var->GetError();

      mError = ss.str();
    } else {
      std::stringstream ss;
      ss << "Unknown member type '" << szMemberType << "' for object '"
         << szName << "'.";
...
bool MetaObjectXmlParser::DefaultsSpecified(
    const tinyxml2::XMLElement *pMember) const {
  const char *szMemberType = pMember->Attribute("type");
  if (nullptr == szMemberType) {
    return false;
  }
  ...
  if (subVar &&
      subVar->GetMetaType() != MetaVariable::MetaVariableType_t::TYPE_REF) {
  ...
  const tinyxml2::XMLElement *cMember = pMember->FirstChildElement();
  while (nullptr != cMember) {
    if (subMembers.find(cMember->Name()) != subMembers.end() &&
        !DefaultsSpecified(cMember)) {
      return false;
    }
...
bool MetaObjectXmlParser::HasCircularReference(
...
std::shared_ptr<MetaObject> MetaObjectXmlParser::GetKnownObject(
    const std::string &object) const {
  auto obj = mKnownObjects.find(object);
  return obj != mKnownObjects.end() ? obj->second : nullptr;
}

std::unordered_map<std::string, std::shared_ptr<MetaObject>>
MetaObjectXmlParser::GetKnownObjects() const {
