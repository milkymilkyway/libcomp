...
Generator::Generator() {
  ...
  ttvfs::CountedPtr<ttvfs::MemFile> pMemoryFile =
      new ttvfs::MemFile("templates.zip", ResourceTemplate,
                         static_cast<uint32_t>(ResourceTemplateSize));
...
std::string Generator::Tab(size_t count) { return std::string(count * 4, ' '); }
...
std::string Generator::GetObjectName(const std::string& fullName,
  ...
  if (pos != fullName.npos) {
    ...
    outNamespace = fullName.substr(0, fullName.length() - (name.length() + 2));
    return name;
  }
  return fullName;
}
...
bool Generator::SaveString(std::ostream& stream, const std::string& s) {
  ...
  stream.write(reinterpret_cast<char*>(&strLength), sizeof(strLength));
