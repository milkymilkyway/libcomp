...
class MetaVariableInt : public MetaVariable {
 ...
 public:
  ...
  void SetDefaultValue(T value) { mDefaultValue = value; }

  T GetMinimumValue() const { return mMinimumValue; }

  void SetMinimumValue(T value) { mMinimumValue = value; }

  T GetMaximumValue() const { return mMaximumValue; }

  void SetMaximumValue(T value) { mMaximumValue = value; }
  ...
  static T StringToValue(const std::string& str, bool* pOK = nullptr) {
    ...
    if (ok) {
      if (!std::numeric_limits<T>::is_integer) {
        ...
        if (ss && static_cast<T>(temp) >= std::numeric_limits<T>::lowest() &&
            static_cast<T>(temp) <= std::numeric_limits<T>::max()) {
  ...
  virtual bool IsCoreType() const { return true; }

  virtual bool IsScriptAccessible() const { return true; }

  virtual bool IsValid() const {
    return mMinimumValue <= mMaximumValue && mMinimumValue <= mDefaultValue &&
  ...
  virtual bool Load(std::istream& stream) {
    ...
    stream.read(reinterpret_cast<char*>(&mDefaultValue), sizeof(mDefaultValue));
    stream.read(reinterpret_cast<char*>(&mMinimumValue), sizeof(mMinimumValue));
    stream.read(reinterpret_cast<char*>(&mMaximumValue), sizeof(mMaximumValue));
  ...
  virtual bool Save(tinyxml2::XMLDocument& doc, tinyxml2::XMLElement& parent,
    ...
    if (0 != GetDefaultValue()) {
      if (std::numeric_limits<T>::is_integer) {
        pVariableElement->SetAttribute(
            "default", std::to_string(GetDefaultValue()).c_str());
      } else {
        std::stringstream ss;
        ss << GetDefaultValue();
    ...
    if (std::numeric_limits<T>::lowest() != GetMinimumValue()) {
      if (std::numeric_limits<T>::is_integer) {
        pVariableElement->SetAttribute(
            "min", std::to_string(GetMinimumValue()).c_str());
      } else {
        std::stringstream ss;
        ss << GetMinimumValue();
    ...
    if (std::numeric_limits<T>::max() != GetMaximumValue()) {
      if (std::numeric_limits<T>::is_integer) {
        pVariableElement->SetAttribute(
            "max", std::to_string(GetMaximumValue()).c_str());
      } else {
        std::stringstream ss;
        ss << GetMaximumValue();
  ...
  virtual std::string GetArgumentType() const { return GetCodeType(); }
  ...
  virtual std::string GetLoadRawCode(const Generator& generator,
    ...
    if (MetaObject::IsValidIdentifier(name)) {
      ...
      code = generator.ParseTemplate(0, "VariableIntLoad", replacements);
    }
  ...
  virtual std::string GetSaveRawCode(const Generator& generator,
    ...
    if (MetaObject::IsValidIdentifier(name)) {
      ...
      code = generator.ParseTemplate(0, "VariableIntSave", replacements);
    }
  ...
  virtual std::string GetXmlLoadCode(const Generator& generator,
    ...
    replacements["@CONVERT_FUNC@"] =
        std::numeric_limits<T>::is_integer ? "ToInteger" : "ToDecimal";
