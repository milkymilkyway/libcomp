...
TEST(MetaVariableType, Array) {
  auto elementType =
      std::shared_ptr<MetaVariableInt<uint8_t>>(new MetaVariableInt<uint8_t>);
  ...
  ASSERT_EQ(
      std::static_pointer_cast<MetaVariableInt<uint8_t>>(var.GetElementType())
          ->GetDefaultValue(),
      std::static_pointer_cast<MetaVariableInt<uint8_t>>(copy.GetElementType())
          ->GetDefaultValue());
...
TEST(MetaVariableType, Enum) {
  ...
  ASSERT_FALSE(var.IsValid()) << "Checking validity with missing requirements.";
  ...
  values = {{"VALUE_1", ""}, {"VALUE_2", "1"}, {"VALUE_3", "1"}};
  ASSERT_FALSE(var.SetValues(values))
      << "Setting invalid values containing an duplicate numeric value.";

  values = {{"VALUE_1", ""}, {"VALUE_2", "2"}, {"VALUE_3", "3"}};
  ASSERT_TRUE(var.SetValues(values)) << "Setting valid values.";
...
TEST(MetaVariableType, List) {
  ...
  ASSERT_EQ(var.GetName(), copy.GetName());
  ASSERT_EQ(var.GetElementType()->GetMetaType(),
            copy.GetElementType()->GetMetaType());
...
TEST(MetaVariableType, Map) {
  auto keyType =
      std::shared_ptr<MetaVariableInt<uint8_t>>(new MetaVariableInt<uint8_t>);
  auto valueType =
      std::shared_ptr<MetaVariableInt<uint16_t>>(new MetaVariableInt<uint16_t>);
  ...
  ASSERT_EQ(var.GetName(), copy.GetName());
  ASSERT_EQ(var.GetKeyElementType()->GetMetaType(),
            copy.GetKeyElementType()->GetMetaType());
...
TEST(MetaVariableType, Reference) {
  ...
  auto defaultedVar =
      std::shared_ptr<MetaVariable>(new MetaVariableInt<uint8_t>);
  std::dynamic_pointer_cast<MetaVariableInt<uint8_t>>(defaultedVar)
      ->SetDefaultValue(5);
  ...
  ASSERT_EQ(var.GetDefaultedVariables().size(),
            copy.GetDefaultedVariables().size());
...
TEST(MetaVariableType, String) {
  ...
  ASSERT_TRUE(var.IsValid()) << "Checking validity after removing the regex.";
...
TEST(MetaVariableInt, StringToValue) {
  ...
  double val =
      libobjgen::MetaVariableInt<double>::StringToValue("-3.14159e10", &ok);
