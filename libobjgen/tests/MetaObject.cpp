...
TEST(MetaObject, Validate) {
  ...
  ASSERT_FALSE(obj.SetName("2Test")) << "Attempting to set an invalid name.";

  ASSERT_TRUE(obj.SetName("Test")) << "Attempting to set a valid name.";
  ...
  ASSERT_FALSE(obj.IsValid()) << "Attempting to validate a non-persistent "
                                 "object with a source location.";
...
TEST(MetaObjectXmlParser, ValidReferenceCheck) {
  ...
  const tinyxml2::XMLElement *pObjectXml =
...
TEST(MetaObjectXmlParser, ScriptEnabledCheck) {
  ...
  tinyxml2::XMLElement *pObjectXml =
      doc.RootElement()->FirstChildElement("object");
  ...
  // Building Object1 or Object2 is not a problem until we try to build the
  // derived object too
  ASSERT_TRUE(parser.FinalizeObjectAndReferences("Object1"));
  ...
  // Object2 should build but there will be no script enabled reference to
  // Object3
  ...
  ASSERT_TRUE(obj3Var &&
              obj3Var->GetMetaType() ==
                  libobjgen::MetaVariable::MetaVariableType_t::TYPE_REF);

  auto obj3Ref =
      std::dynamic_pointer_cast<libobjgen::MetaVariableReference>(obj3Var);
  ASSERT_FALSE(obj3Ref->IsScriptReference());

  // Reset and make the referenced object script enabled as well
  parser = MetaObjectXmlParser();

  pObjectXml = doc.RootElement()
  ...
  // Object2 should now build and also have a script enabled reference to
  // Object3
  ASSERT_TRUE(parser.FinalizeObjectAndReferences("Object2"));
...
TEST(MetaObjectXmlParser, ParseAllObjectAttributes) {
  auto xml =
      ...
      "<object name='Object3' namespace='ns2' baseobject='Object2' "
      "persistent='false'/>"
      "</objects>";
