...
class MariaDBAccount : public objects::Account {
 public:
  ...
  static void RegisterPersistentType() {
    RegisterType(typeid(MariaDBAccount), MariaDBAccount::GetMetadata(),
...
TEST(MariaDB, ObjectBindIndex) {
  ...
  EXPECT_TRUE(db.Use());
  EXPECT_TRUE(
      db.Execute("CREATE TABLE objects ( uid VARCHAR(36) PRIMARY KEY, sortby "
                 "int, data blob, txt text );"));

  DatabaseQuery q = db.Prepare(
      "INSERT INTO objects ( uid, sortby, data, txt ) VALUES ( ?, ?, ?, ? );");
  ...
  q = db.Prepare(
      "SELECT uid, sortby, data, txt FROM objects ORDER BY sortby ASC;");
  EXPECT_TRUE(q.IsValid());
  EXPECT_TRUE(q.Execute());
...
TEST(MariaDB, ObjectBindName) {
  ...
  DatabaseQuery q = db.Prepare(
      "INSERT INTO objects ( uid, sortby, data, txt ) VALUES ( :uid, :sortby, "
      ":data, :txt );");
  EXPECT_TRUE(q.IsValid());
