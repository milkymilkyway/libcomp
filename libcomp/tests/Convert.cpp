...
TEST(String, CP1252) {
  ...
  unsigned char encodedString[] = {
      0x54, 0x68, 0x69, 0x73, 0x20, 0x69, 0x73, 0x20, 0x43, 0x50, 0x2d,
      0x31, 0x32, 0x35, 0x32, 0x20, 0x65, 0x6e, 0x63, 0x6f, 0x64, 0x69,
      0x6e, 0x67, 0x3a, 0x20, 0xa9, 0xc6, 0xfc, 0xd8, 0x00};

  std::vector<char> encodedData(
      reinterpret_cast<char *>(encodedString),
      reinterpret_cast<char *>(encodedString) + sizeof(encodedString));

  std::vector<char> testData =
      Convert::ToEncoding(Convert::ENCODING_CP1252, decodedString);

  ASSERT_EQ(testData.size(), encodedData.size());

  EXPECT_EQ(memcmp(encodedString, &testData[0], sizeof(encodedString)), 0);
  EXPECT_EQ(Convert::FromEncoding(Convert::ENCODING_CP1252, &encodedData[0]),
            decodedString);

  testData =
      Convert::ToEncoding(Convert::ENCODING_CP1252, decodedString, false);

  ASSERT_EQ(testData.size(), encodedData.size() - 1);

  EXPECT_EQ(memcmp(encodedString, &testData[0], sizeof(encodedString) - 1), 0);
  EXPECT_EQ(Convert::FromEncoding(Convert::ENCODING_CP1252, &encodedData[0],
...
TEST(String, CP932) {
  ...
  unsigned char encodedString[] = {
      0x54, 0x68, 0x69, 0x73, 0x20, 0x69, 0x73, 0x20, 0x43, 0x50, 0x2d,
      0x39, 0x33, 0x32, 0x20, 0x65, 0x6e, 0x63, 0x6f, 0x64, 0x69, 0x6e,
      0x67, 0x3a, 0x20, 0x93, 0xfa, 0x96, 0x7b, 0x8c, 0xea, 0x82, 0xaa,
      0x91, 0xe5, 0x8d, 0x44, 0x82, 0xab, 0x81, 0x49, 0x00};
  ...
  std::vector<char> testData =
      Convert::ToEncoding(Convert::ENCODING_CP932, decodedString);
  ...
  testData = Convert::ToEncoding(Convert::ENCODING_CP932, decodedString, false);
