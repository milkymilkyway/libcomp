...
/// ID to start with additional server specific log components.
#define LOG_SERVER_SPECIFIC_START_ID (1000)

/// Generic type for either BaseLogComponent_t or LogComponent_t
...
class BaseLog {
 ...
 public:
  ...
  /**
   * Return a pointer to the Log singleton.
 ...
 protected:
   ...
   */
  EnumMap<GenericLogComponent_t, Level_t> mComponentLogLevels;
...
class LogMessage {
 public:
   ...
   */
  explicit LogMessage(GenericLogComponent_t comp, BaseLog::Level_t level)
      : mComponent(comp),
        mLevel(level),
...
class LogMessageImpl : public LogMessage {
 public:
  ...
  explicit LogMessageImpl(GenericLogComponent_t comp, BaseLog::Level_t level,
...
class LogMessageFixed : public LogMessage {
 public:
   ...
   */
  explicit LogMessageFixed(GenericLogComponent_t comp, BaseLog::Level_t level,
                           const String& msg)
 ...
 */
#define BASELOG_FUNCTIONS(comp)                                               \
  BASELOG_FUNCTION(Log##comp##Debug, comp, libcomp::BaseLog::LOG_LEVEL_DEBUG) \
  BASELOG_FUNCTION(Log##comp##Info, comp, libcomp::BaseLog::LOG_LEVEL_INFO)   \
  BASELOG_FUNCTION(Log##comp##Warning, comp,                                  \
                   libcomp::BaseLog::LOG_LEVEL_WARNING)                       \
  BASELOG_FUNCTION(Log##comp##Error, comp, libcomp::BaseLog::LOG_LEVEL_ERROR) \
  BASELOG_FUNCTION(Log##comp##Critical, comp,                                 \
                   libcomp::BaseLog::LOG_LEVEL_CRITICAL)

// Add a log function set for each component here!
BASELOG_FUNCTIONS(Connection)
BASELOG_FUNCTIONS(Crypto)
BASELOG_FUNCTIONS(Database)
BASELOG_FUNCTIONS(DataStore)
BASELOG_FUNCTIONS(DataSyncManager)
BASELOG_FUNCTIONS(General)
BASELOG_FUNCTIONS(Packet)
BASELOG_FUNCTIONS(ScriptEngine)
BASELOG_FUNCTIONS(Server)
