...
ScriptEngine& ScriptEngine::Using<DataSyncManager>() {
  if (!BindingExists("DataSyncManager", true)) {
    ...
    binding.Func("UpdateRecord", &DataSyncManager::UpdateRecord)
...
bool DataSyncManager::RegisterConnection(
...
bool DataSyncManager::RemoveConnection(
    const std::shared_ptr<InternalConnection>& connection) {
  std::lock_guard<std::mutex> lock(mLock);
  for (auto pair : mConnections) {
    if (pair.first == connection) {
...
void DataSyncManager::SyncOutgoing() {
  ...
  LogDataSyncManagerDebug([updateCount, removeCount]() {
    return libcomp::String(
               "Synchronizing %1 outbound update(s) and %2 outbound "
               "remove(s).\n")
        .Arg(updateCount)
        .Arg(removeCount);
  });
...
bool DataSyncManager::SyncIncoming(libcomp::ReadOnlyPacket& p,
  ...
  std::shared_ptr<libcomp::DataSyncManager::ObjectConfig> config;
  {
    ...
    String lType(p.ReadString16Little(libcomp::Convert::ENCODING_UTF8, true));
    ...
    if (configIter == mRegisteredTypes.end()) {
      LogDataSyncManagerWarning([&]() {
        return String("Ignoring sync request for unregistered type: %1\n")
            .Arg(lType);
      });
    ...
    if (!isPersistent) {
      if (!config->BuildHandler) {
        LogDataSyncManagerError([&]() {
          return String(
                     "Non-persistent object type without a registered build "
                     "handler encountered: %1\n")
              .Arg(type);
        });
        ...
        LogDataSyncManagerError([&]() {
          return String(
                     "Object type without a registered update or sync complete "
                     "handler encountered: %1\n")
              .Arg(type);
        });
    ...
    if (recordsCount > 0) {
      if (source.IsEmpty()) {
        LogDataSyncManagerDebug([type, recordsCount, source]() {
          return libcomp::String("Synchronizing %1 inbound %2 update(s).\n")
              .Arg(recordsCount)
              .Arg(type);
        });
      } else {
        LogDataSyncManagerDebug([type, recordsCount, source]() {
          return libcomp::String(
                     "Synchronizing %1 inbound %2 update(s) from source: %3\n")
              .Arg(recordsCount)
              .Arg(type)
              .Arg(source);
        });
    ...
    if (isPersistent) {
      ...
      for (uint16_t k = 0; k < recordsCount; k++) {
        String uidStr(
            p.ReadString16Little(libcomp::Convert::ENCODING_UTF8, true));

        libobjgen::UUID uid(uidStr.C());
        if (!uid.IsNull()) {
          auto obj = PersistentObject::LoadObjectByUUID(
              typeHash, mRegisteredTypes[type]->DB, uid, true);
          if (obj) {
          ...
          LogDataSyncManagerError([&]() {
            return String(
                       "Null UID encountered for updated sync record of type: "
                       "%1\n")
                .Arg(type);
          });
        }
      ...
      for (uint16_t k = 0; k < recordsCount; k++) {
        ...
        if (!obj->LoadPacket(p, false)) {
          LogDataSyncManagerError([&]() {
            return String(
                       "Invalid update data stream received from "
                       "non-persistent object of type: %1\n")
                .Arg(type);
          });
    ...
    if (customHandler || completeHandler) {
      for (auto obj : records) {
        ...
        if (customHandler) {
          int8_t result =
              config->UpdateHandler(*this, lType, obj, false, source);
          if (result == SYNC_UPDATED) {
            ...
            LogDataSyncManagerError([&]() {
              return String("Failed to sync update of record of type: %1\n")
                  .Arg(type);
            });
    ...
    if (recordsCount > 0) {
      if (source.IsEmpty()) {
        LogDataSyncManagerDebug([type, recordsCount, source]() {
          return libcomp::String("Synchronizing %1 inbound %2 remove(s).\n")
              .Arg(recordsCount)
              .Arg(type);
        });
      } else {
        LogDataSyncManagerDebug([type, recordsCount, source]() {
          return libcomp::String(
                     "Synchronizing %1 inbound %2 remove(s) from source: %3\n")
              .Arg(recordsCount)
              .Arg(type)
              .Arg(source);
        });
    ...
    if (isPersistent) {
      ...
      for (uint16_t k = 0; k < recordsCount; k++) {
        ...
        if (!uid.IsNull()) {
          ...
          LogDataSyncManagerError([&]() {
            return String(
                       "Null UID encountered for removed sync record of type: "
                       "%1\n")
                .Arg(type);
          });
        }
      ...
      for (uint16_t k = 0; k < recordsCount; k++) {
        ...
        if (!obj->LoadPacket(p, false)) {
          LogDataSyncManagerError([&]() {
            return String(
                       "Invalid remove data stream received from "
                       "non-persistent object of type: %1\n")
                .Arg(type);
          });
    ...
    if (customHandler || completeHandler) {
      for (auto obj : records) {
        ...
        if (customHandler) {
          int8_t result =
              config->UpdateHandler(*this, lType, obj, true, source);
          if (result == SYNC_UPDATED) {
            if (config->ServerOwned) {
              // Queue up to report to the other server(s)
            ...
            LogDataSyncManagerError([&]() {
              return String("Failed to sync removal of record of type: %1\n")
                  .Arg(type);
            });
...
bool DataSyncManager::UpdateRecord(
  ...
  if (record != nullptr && configIter != mRegisteredTypes.end()) {
    if ((configIter->second->ServerOwned ||
      ...
      int8_t result =
          configIter->second->UpdateHandler(*this, type, record, false, "");
...
bool DataSyncManager::SyncRecordUpdate(
    const std::shared_ptr<libcomp::Object>& record,
    const libcomp::String& type) {
...
bool DataSyncManager::RemoveRecord(
    const std::shared_ptr<libcomp::Object>& record,
    const libcomp::String& type) {
  std::lock_guard<std::mutex> lock(mLock);
  ...
  if (record != nullptr && configIter != mRegisteredTypes.end()) {
    if ((configIter->second->ServerOwned ||
        ...
        configIter->second->UpdateHandler) {
      int8_t result =
          configIter->second->UpdateHandler(*this, type, record, true, "");
      if (result == SYNC_HANDLED) {
        return false;
      }
    }
...
bool DataSyncManager::SyncRecordRemoval(
    const std::shared_ptr<libcomp::Object>& record,
    const libcomp::String& type) {
...
void DataSyncManager::QueueOutgoing(
    const libcomp::String& type,
    const std::shared_ptr<InternalConnection>& connection,
...
void DataSyncManager::WriteOutgoingRecord(
  ...
  if (isPersistent) {
    // Write the UUID
    auto pObj = std::dynamic_pointer_cast<PersistentObject>(record);
...
void DataSyncManager::WriteOutgoingRecords(
  ...
  if (isPersistent) {
    // Write the UUID
    for (auto obj : records) {
      auto pObj = std::dynamic_pointer_cast<PersistentObject>(obj);
      p.WriteString16Little(libcomp::Convert::ENCODING_UTF8,
                            pObj->GetUUID().ToString(), true);
    }
  } else {
