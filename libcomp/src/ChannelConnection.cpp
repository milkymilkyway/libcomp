  ...
  if (STATUS_ENCRYPTED == mStatus) {
    ...
    while (!packetOK && 2 > retryCount++) {
      ...
      int32_t originalSize =
          static_cast<int32_t>(finalPacket.Size() - headerSize);
      int compressedSize;
    ...
    if (packetOK) {
      ...
      if (nullptr != mCaptureFile) {
        ...
        uint32_t realSize =
            capturePacket.Size() - 2 * static_cast<uint32_t>(sizeof(uint32_t));
        ...
        mCaptureFile->write(capturePacket.ConstData(), size);
...
uint32_t ChannelConnection::GetHeaderSize() { return CHANNEL_HEADER_SIZE; }
