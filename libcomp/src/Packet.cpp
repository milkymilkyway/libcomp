...
Packet::Packet(const Packet& other)
    : ReadOnlyPacket(other.mPosition, other.mSize, nullptr, nullptr) {
  Allocate();
...
Packet::Packet(Packet&& other)
    : ReadOnlyPacket(other.mPosition, other.mSize, other.mData,
                     other.mDataRef) {
  other.mPosition = 0;
  other.mSize = 0;
...
void Packet::GrowPacket(uint32_t sz) {
  ...
  if (newSize < mSize) {
    ...
    PACKET_EXCEPTION(String("Attempted to grow packet by %1 bytes; however, "
                            "doing so would exceed MAX_PACKET_SIZE")
                         .Arg(sz),
                     this);
  } else {
...
void Packet::WriteS16Little(int16_t value) {
  // Convert the value to little endian byte order and then write it.
  WriteS16((int16_t)htole16((uint16_t)value));
}
...
void Packet::WriteS32Little(int32_t value) {
  // Convert the value to little endian byte order and then write it.
  WriteS32((int32_t)htole32((uint32_t)value));
}
...
void Packet::WriteS64Little(int64_t value) {
  // Convert the value to little endian byte order and then write it.
  WriteS64((int64_t)htole64((uint64_t)value));
}
...
char* Packet::Direct(uint32_t sz) {
  ...
  if (MAX_PACKET_SIZE < sz) {
    PACKET_EXCEPTION(
        String("Attempted to directly access the first %1 bytes of the packet; "
               "however, this size exceeds the MAX_PACKET_SIZE")
            .Arg(sz),
        this);
  }
  ...
  if (mPosition > sz) {
    PACKET_EXCEPTION(
        String("Attempted to directly access the first %1 bytes of the packet; "
               "however, this size exceeds the size of the packet")
            .Arg(sz),
        this);
  }
...
int32_t Packet::Decompress(int32_t sz) {
  ...
  if ((mPosition + (uint32_t)sz) > mSize) {
    PACKET_EXCEPTION(String("Attempted to decompress %1 bytes from the packet; "
                            "however, this size exceeds the size of the packet")
                         .Arg(sz),
                     this);
  }
  ...
  // Decompress the data
  int32_t written = Compress::Decompress(pData, mData + mPosition, sz,
                                         (int32_t)(MAX_PACKET_SIZE - mSize));
...
int32_t Packet::Compress(int32_t sz) {
  ...
  // remaining packet data. If not, throw an exception.
  if ((mPosition + (uint32_t)sz) > mSize) {
    PACKET_EXCEPTION(String("Attempted to compress %1 bytes from the packet; "
                            "however, this size exceeds the size of the packet")
                         .Arg(sz),
                     this);
  }
  ...
  // Compress the data
  int32_t written = Compress::Compress(pData, mData + mPosition, sz,
                                       (int32_t)(MAX_PACKET_SIZE - mSize));
...
ScriptEngine& ScriptEngine::Using<Packet>() {
  if (!BindingExists("Packet")) {
    ...
    binding.Func("WriteBlank", &Packet::WriteBlank)
        .Func("WriteU16Little", &Packet::WriteU16Little)
        .Func<void (Packet::*)(const std::vector<char>&)>(
            "WriteArray", &Packet::WriteArray);  // Last call to binding
