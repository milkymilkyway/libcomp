...
bool DatabaseQuerySQLite3::Prepare(const String& query) {
  ...
  mStatus = sqlite3_prepare_v2(mDatabase, query.C(), len, &mStatement, nullptr);
...
bool DatabaseQuerySQLite3::Bind(size_t index, const String& value) {
  ...
  mStatus =
      sqlite3_bind_text(mStatement, idx, value.C(), len, SQLITE_TRANSIENT);
  return IsValid();
}
...
bool DatabaseQuerySQLite3::Bind(size_t index, const std::vector<char>& value) {
  if (nullptr != mStatement) {
    ...
    mStatus =
        sqlite3_bind_blob(mStatement, idx, &value[0], size, SQLITE_TRANSIENT);
  } else {
    return false;
  }
...
bool DatabaseQuerySQLite3::Bind(
    size_t index,
...
bool DatabaseQuerySQLite3::GetRows(
...
int DatabaseQuerySQLite3::GetStatus() const { return mStatus; }
...
bool DatabaseQuerySQLite3::GetResultColumnIndex(const String& name,
                                                size_t& index) const {
  auto iter =
