  ...
  if (ROLE_CLIENT != GetRole() || ConnectionMode_t::MODE_NORMAL == mMode) {
    ...
    switch (mMode) {
      case ConnectionMode_t::MODE_PING: {
        mPacketParser =
            static_cast<PacketParser_t>(&LobbyConnection::ParseExtension);
...
bool LobbyConnection::ParseExtensionConnection(libcomp::Packet& packet) {
  ...
  if (0 == packet.Left() && 2 == first && 8 == second) {
    ...
    if (ROLE_CLIENT == GetRole()) {
      ...
      SendMessage([](const std::shared_ptr<libcomp::TcpConnection>&) {
        return new libcomp::Message::WorldNotification(libcomp::String(), 0);
      });
    } else {
      // This is a ping, issue a pong.
      LogConnectionDebugMsg("Got a world server notification.\n");

      uint16_t worldServerPort = static_cast<uint16_t>((first >> 16) & 0xFFFF);

      // Set the name of the connection to the world's port.
      SetName(String("world_port:%1").Arg(worldServerPort));

      SendMessage([&](const std::shared_ptr<libcomp::TcpConnection>&) {
        return new libcomp::Message::WorldNotification(GetRemoteAddress(),
                                                       worldServerPort);
      });
...
void LobbyConnection::SetListenPort(uint16_t port) { mListenPort = port; }

uint16_t LobbyConnection::GetListenPort() const { return mListenPort; }
