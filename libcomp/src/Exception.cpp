...
static size_t baseLen =
    strlen(__FILE__) - strlen("libcomp/libcomp/src/Exception.cpp");
...
Exception::Exception(const String &msg, const String &f, int l)
  ...
  for (USHORT i = 0; i < frameCount; ++i) {
    ...
    if (TRUE == symInit &&
        TRUE == SymFromAddr(GetCurrentProcess(), (DWORD64)backtraceAddresses[i],
                            &displacement, pInfo)) {
...
#define backtrace_size_t int32_t
  ...
  if (backtraceSize > 0) {
    ...
    char** backtraceSymbols =
        backtrace_symbols(backtraceAddresses, backtraceSize);
...
LONG WINAPI TopLevelExceptionHandler(PEXCEPTION_POINTERS pExceptionInfo) {
  ...
  SYMBOL_INFO *symbol =
      (SYMBOL_INFO *)new uint8_t[sizeof(SYMBOL_INFO) + 256 + 1];
  ...
  while (StackWalk64(machine_type, GetCurrentProcess(), GetCurrentThread(),
    ...
    if (SymFromAddr(process, address, &displacement, symbol)) {
      ...
      ss << "(" << symbol->Name << "+0x" << std::hex << (int64_t)displacement
         << ")";

      DWORD lineDisplacement;
      IMAGEHLP_LINE64 line;

      if (TRUE == SymGetLineFromAddr64(GetCurrentProcess(), (ULONG64)address,
                                       &lineDisplacement, &line)) {
        ss << " " << line.FileName << ":" << std::dec << line.LineNumber;
      }
  ...
  LogGeneralCriticalMsg(
      "The server has crashed with an unhandled exception. A backtrace will "
      "follow.\n");

  LogGeneralCritical([&]() { return String("Backtrace: %1\n").Arg(ss.str()); });
...
String Exception::File() const {
  ...
  if (mFile.Length() > baseLen &&
      mFile.Left(baseLen) == String(__FILE__).Left(baseLen)) {
...
String Exception::Message() const { return mMessage; }

std::list<String> Exception::Backtrace() const { return mBacktrace; }
