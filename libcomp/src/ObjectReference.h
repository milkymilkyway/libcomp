...
class ObjectReferenceData {
 public:
   ...
   */
  ObjectReferenceData() { mLoadFailed = false; }
  ...
  ObjectReferenceData(const std::shared_ptr<PersistentObject>& ref,
      ...
      : mRef(ref), mUUID(ref != nullptr ? ref->GetUUID() : uuid) {
    mLoadFailed = false;
  }
...
class ObjectReference {
 public:
   ...
   */
  bool IsNull() const { return mData == sNull; }
   ...
   */
  operator bool() const { return nullptr != mData->mRef; }

  /**
   * Get the UUID of the reference.
   * @return UUID of the reference
   */
  const libobjgen::UUID& GetUUID() const { return mData->mUUID; }
 ...
 protected:
  ...
  bool LoadReference(const std::shared_ptr<Database>& db = nullptr,
    ...
    if (!mData->mLoadFailed && (reload || nullptr == mData->mRef) &&
      ...
      bool dbLoad = nullptr != db && typeid(PersistentObject) != typeid(T);
      std::shared_ptr<PersistentObject> pRef;
      if (dbLoad) {
        pRef = std::dynamic_pointer_cast<PersistentObject>(
            PersistentObject::LoadObjectByUUID<T>(db, uuid, reload));
      } else {
  ...
  void SetReference(const libobjgen::UUID& uuid,
    ...
    if (!uuid.IsNull()) {
      ...
      if (ref != nullptr) {
        mData =
            std::shared_ptr<ObjectReferenceData>(new ObjectReferenceData(ref));
      } else {
        mData = sNull;
      }
    }
  ...
  static std::unordered_map<std::string, std::shared_ptr<ObjectReferenceData>>
      sData;
