...
SpawnThread::SpawnThread(DayCare* pJuvy, bool printDetails,
                         std::function<void()> onDetain)
    : mPrintDetails(printDetails), mDayCare(pJuvy), mOnDetain(onDetain) {
  mThread = new std::thread([](SpawnThread* pThread) { pThread->Run(); }, this);
}
...
void SpawnThread::RequestExit() { QueueChild(nullptr); }
