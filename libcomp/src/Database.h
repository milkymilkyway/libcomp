...
class Database : public std::enable_shared_from_this<Database> {
 public:
  ...
  virtual bool DeleteObjects(
      std::list<std::shared_ptr<PersistentObject>>& objs) = 0;
  ...
  /**
   * Check if the config is for the default database type.  Non-default
   * connections are only responsible for verifying the schema of their own
   * tables where-as default database connections may also have additional
   * tables to maintain.
 ...
 protected:
  ...
  virtual bool ProcessStandardChangeSet(
      const std::shared_ptr<DBStandardChangeSet>& changes) = 0;
  ...
  virtual bool ProcessOperationalChangeSet(
      const std::shared_ptr<DBOperationalChangeSet>& changes) = 0;
 ...
 private:
  ...
  std::unordered_map<std::string, std::shared_ptr<DBStandardChangeSet>>
      mTransactionQueue;
