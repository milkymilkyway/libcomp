...
bool Crypto::DecryptFile(std::vector<char> &data)
{
    ...
    if(sizeof(EncryptedFileHeader_t) < data.size())
    {
                     ...
                     sizeof(pHeader->magic)))
        {
            ...
            // Remove the header.
            data.erase(
                data.begin(), data.begin() + sizeof(EncryptedFileHeader_t));
...
bool Crypto::EncryptFile(std::vector<char> &data)
{
    ...
    memcpy(
        &header.magic[0], Config::ENCRYPTED_FILE_MAGIC, sizeof(header.magic));
...
bool Crypto::EncryptFile(const std::string &path, const std::vector<char> &data)
{
...
String Crypto::GenDiffieHellman(
    const String &g, const String &p, const String &a, int outputSize)
{
    (void)g;
    (void)p;
    (void)a;
...
void Crypto::Blowfish::Decrypt(
    std::vector<char> &data, std::vector<char>::size_type realSize)
{
    std::vector<char>::size_type size = data.size();
    char *pData = &data[0];

    if((0 == realSize || realSize <= size) && 0 == (size % BLOWFISH_BLOCK_SIZE))
    {
        // Decrypt each full block.
        while(BLOWFISH_BLOCK_SIZE <= size)
        {
...
void Crypto::Blowfish::EncryptCbc(
    uint64_t &initializationVector, std::vector<char> &data)
{
    std::vector<char>::size_type size = data.size();
 ...
 */
static int DiffieHellmanRandom(
    void *rng_state, unsigned char *output, size_t len)
{
    (void)rng_state;
...
String Crypto::DiffieHellman::BufferToHexString(
    const uint8_t *pBuffer, size_t bufferSize)
{
...
String Crypto::DiffieHellman::GetPrime() const
{
    return d->mPrime;
}
...
String Crypto::DiffieHellman::GeneratePublic()
{
    ...
    if(0 == mbedtls_dhm_make_public(&d->mContext, DH_KEY_BIT_SIZE / 8,
                pPublicBinary, sizeof(pPublicBinary), DiffieHellmanRandom,
                NULL))
    {
...
String Crypto::DiffieHellman::GetPublic() const
{
    return d->mPublic;
}
    ...
    const String &otherPublic)
{
    ...
    if(0 == mbedtls_dhm_read_public(&d->mContext, &otherPublicBinary[0],
        ...
        0 == mbedtls_dhm_calc_secret(&d->mContext, pSecretBinary,
                 sizeof(pSecretBinary), &olen, DiffieHellmanRandom, NULL) &&
...
bool Crypto::DiffieHellman::IsValid() const
{
    return d->mValid;
}
...
{
    BF_set_key(
        &d->key, (int)dataSize, reinterpret_cast<const unsigned char *>(pData));
}
...
void Crypto::Blowfish::DecryptCbc(
    std::vector<char> &data, std::vector<char>::size_type realSize)
{
    uint64_t initializationVector =
        *reinterpret_cast<const uint64_t *>(Config::ENCRYPTED_FILE_IV);
