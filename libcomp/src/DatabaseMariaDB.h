...
class DatabaseMariaDB : public Database {
 ...
 private:
   ...
   * @param databaseName Name of the database to connect to on the configured
   * host
   * @return true if the connection succeeded, false if it did not
   */
  bool ConnectToDatabase(MYSQL*& connection,
  ...
  /// Pointers to the MariaDB representation of a database connection per
  /// thread. References to the pointers in this collection are passed around
  /// for use throughout the class as they are not removed or modified until the
  /// database is closed.
