...
class PersistentObject : public Object {
 ...
 public:
  ...
  typedef std::unordered_map<size_t, std::shared_ptr<libobjgen::MetaObject>>
      TypeMap;
   ...
   */
  template <class T>
  static std::list<std::shared_ptr<T>> LoadAll(
   ...
   */
  template <class T>
  static std::shared_ptr<T> LoadObjectByUUID(
   ...
   */
  template <class T>
  static std::shared_ptr<T> New(bool doRegister = false) {
    if (std::is_base_of<PersistentObject, T>::value) {
   ...
   */
  template <class T>
  static std::list<std::shared_ptr<PersistentObject>> ToList(
      std::list<std::shared_ptr<T>> objList) {
    std::list<std::shared_ptr<PersistentObject>> converted;
    if (std::is_base_of<PersistentObject, T>::value) {
      for (auto obj : objList) {
        converted.push_back(std::dynamic_pointer_cast<PersistentObject>(obj));
      }
    }
 ...
 protected:
   ...
   * @return Pointer to the object or nullptr if it doesn't exist
   */
  static std::shared_ptr<PersistentObject> LoadObject(
      size_t typeHash, const std::shared_ptr<Database>& db,
      DatabaseBind* pValue);
 ...
 private:
  ...
  static std::unordered_map<std::string, std::weak_ptr<PersistentObject>>
      sCached;
  ...
  static std::unordered_map<size_t, std::function<PersistentObject*()>>
      sFactory;
