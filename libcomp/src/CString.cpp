...
String String::Left(size_t length) const {
  if (0 == length) {
    ...
    for (it = d->mString.cbegin(); 0 < len && it != d->mString.cend(); ++it) {
      if ((*it & 0xC0) != 0x80) {
        len--;
      }
    }
    ...
    StringData* pData =
        new StringData(std::string(d->mString.cbegin(), --it), length);
...
String String::Right(size_t length) const {
  if (0 == length) {
    ...
    for (it = d->mString.crbegin(); 0 < len && it != d->mString.crend(); ++it) {
      if ((*it & 0xC0) != 0x80) {
        len--;
      }
    }
    ...
    StringData* pData =
        new StringData(std::string(it.base(), d->mString.cend()), length);
...
String String::Mid(size_t position, size_t count) const {
  if (position >= d->mLength) {
    ...
    for (begin = d->mString.cbegin();
         0 < position && begin != d->mString.cend(); ++begin) {
      if ((*begin & 0xC0) != 0x80) {
        position--;
      }
    }
...
String::CodePoint String::At(size_t position) const {
  if (position >= d->mLength) {
    ...
    if (0 == (bytes[0] & 0x80)) {
      ...
      cp = (CodePoint)(((bytes[0] & 0x1F) << 6) | (bytes[1] & 0x3F));
    } else if (0xE0 == (bytes[0] & 0xF0)) {
      bytes[1] = *(++begin);
      bytes[2] = *(++begin);

      cp = (CodePoint)(((bytes[0] & 0x1F) << 12) | ((bytes[1] & 0x3F) << 6) |
                       (bytes[2] & 0x3F));
    } else {
      bytes[1] = *(++begin);
      bytes[2] = *(++begin);
      ...
      cp = (CodePoint)(((bytes[0] & 0x0F) << 18) | ((bytes[1] & 0x3F) << 12) |
...
std::string String::ToUtf8() const { return d->mString; }
...
size_t String::Length() const { return d->mLength; }

size_t String::Size() const { return d->mString.size(); }

bool String::IsEmpty() const { return 0 == d->mLength; }

void String::Clear() { d = std::shared_ptr<StringData>(new StringData); }
...
bool String::Matches(const libcomp::String& expression,
  ...
  if (std::regex_match(s, match, std::regex(expression.ToUtf8()))) {
    matches.resize(
        static_cast<std::vector<libcomp::String>::size_type>(match.size()));

    int i = 0;

    for (auto m : match) {
      matches[static_cast<std::vector<libcomp::String>::size_type>(i++)] =
          libcomp::String(m);
    }
...
const char* String::C() const { return d->mString.c_str(); }

bool String::IsReportingBadArguments() { return mBadArgumentReporting; }
