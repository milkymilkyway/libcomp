/**
 * @file libcomp/src/BaseScriptEngine.h
 * @ingroup libcomp
 *
...
#ifndef LIBCOMP_SRC_BASESCRIPTENGINE_H
#define LIBCOMP_SRC_BASESCRIPTENGINE_H
 ...
 */
class BaseScriptEngine : public std::enable_shared_from_this<BaseScriptEngine> {
 public:
  /**
   * Create the VM.
   * @param useRawPrint Set this to not prefix messages with "SQUIRREL: ".
   */
  BaseScriptEngine(bool useRawPrint = false);

  /**
   * Clean up the VM.
   */
  virtual ~BaseScriptEngine();
   ...
   */
  std::shared_ptr<BaseScriptEngine> Self();

  /**
   * Get a pointer to the script engine.
   * @returns Pointer to the script engine.
   */
  std::shared_ptr<const BaseScriptEngine> Self() const;
   ...
   * @returns Pointer to the script engine.
   */
  static std::shared_ptr<BaseScriptEngine> Self(HSQUIRRELVM vm);
 ...
 protected:
   ...
   *  now instead of waiting until calling @ref BaseScriptEngine::Bind. Useful
   ...
   */
  virtual void InitializeBuiltins();

  /**
   * Initialize the server specific database built-in script modules.
   */
  virtual void InitializeDatabaseBuiltins() = 0;

  /**
   * Initialize the server specific server built-in script modules.
   */
  virtual void InitializeServerBuiltins() = 0;

  /**
   * Initialize the server specific other built-in script modules.
   */
  virtual void InitializeOtherBuiltins() {}

  /// The Sqrat VM
  HSQUIRRELVM mVM;

  /// Bindings that have already been made to objects via @ref
  /// BaseScriptEngine::Using
  ...
  static std::unordered_map<
      std::string,
      std::function<bool(BaseScriptEngine&, const std::string& module)>>
      mModules;
};
...
#endif  // LIBCOMP_SRC_BASESCRIPTENGINE_H
