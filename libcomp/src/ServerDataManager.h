...
class ServerDataManager {
 public:
   ...
   */
  const std::shared_ptr<objects::ServerZone> GetZoneData(
      uint32_t id, uint32_t dynamicMapID, bool applyPartials = false,
   ...
   * @return Pointer to the server zone instance variant matching the specified
   * id
   */
   ...
   */
  bool LoadData(DataStore* pDataStore, DefinitionManager* definitionManager);
   ...
   */
  std::list<std::shared_ptr<ServerScript>> LoadScripts(
   ...
   * @return List of all actions
   */
  static std::list<std::shared_ptr<objects::Action>> GetAllZonePartialActions(
      const std::shared_ptr<objects::ServerZonePartial>& partial,
      bool includeNested);
 ...
 private:
  ...
  std::shared_ptr<T> GetObjectByID(
      K id, std::unordered_map<K, std::shared_ptr<T>>& data) {
  ...
  bool LoadObjects(gsl::not_null<DataStore*> pDataStore,
    ...
    for (auto path : files) {
      if (path.Matches("^.*\\.xml$")) {
        if (LoadObjectsFromFile<T>(pDataStore, path, definitionManager)) {
          loaded = true;
        } else {
          return false;
        }
  ...
  bool LoadObjectsFromFile(gsl::not_null<DataStore*> pDataStore,
    ...
    if (tinyxml2::XML_SUCCESS != objsDoc.Parse(&data[0], data.size())) {
      return false;
    }
   ...
   */
  template <class T>
  bool LoadObject(const tinyxml2::XMLDocument& doc,
   ...
   */
  std::list<libcomp::String> GetInvalidEventIDs(
  ...
  std::unordered_map<
      uint32_t,
      std::unordered_map<uint32_t, std::shared_ptr<objects::ServerZone>>>
      mZoneData;
  ...
  std::unordered_map<uint32_t, std::shared_ptr<objects::ServerZoneInstance>>
      mZoneInstanceData;

  /// Map of server zone instance variant defintions by definition ID
  std::unordered_map<uint32_t,
                     std::shared_ptr<objects::ServerZoneInstanceVariant>>
  ...
  std::unordered_map<uint32_t, std::shared_ptr<objects::ServerZonePartial>>
      mZonePartialData;
  ...
  std::unordered_map<std::string, std::shared_ptr<objects::Event>> mEventData;

  /// Map of shops by definition ID
  std::unordered_map<uint32_t, std::shared_ptr<objects::ServerShop>> mShopData;

  /// List of all COMP shop definition IDs
  std::list<uint32_t> mCompShopIDs;

  /// Map of AI logic groups by definition ID
  std::unordered_map<uint16_t, std::shared_ptr<objects::AILogicGroup>>
      mAILogicGroups;

  /// Map of demon familiarity type entries by definition ID
  std::unordered_map<int32_t, std::shared_ptr<objects::DemonFamiliarityType>>
  ...
  std::unordered_map<uint32_t, std::shared_ptr<objects::DemonQuestReward>>
      mDemonQuestRewardData;

  /// Map of drop sets by definition ID
  std::unordered_map<uint32_t, std::shared_ptr<objects::DropSet>> mDropSetData;
  ...
  std::unordered_map<uint32_t, std::shared_ptr<objects::DropSet>>
      mRedefineDropSetData;

  /// Map of drop set definition IDs to drops loaded from APPEND drop sets.
  /// These are loaded into the actual dropset by
  std::unordered_map<uint32_t, std::list<std::shared_ptr<objects::ItemDrop>>>
      mPendingMergeDrops;
