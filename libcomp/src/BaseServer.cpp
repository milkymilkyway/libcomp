...
bool BaseServer::Initialize() {
  ...
  if (nullptr == GetDiffieHellman()) {
    LogServerErrorMsg(
        "No DH key pair set in the config file, it will need to be generated "
        "on startup.\n");
  }
  ...
  if (0 == mConfig->DataStoreCount()) {
    LogServerCriticalMsg("At least one data store path must be specified.\n");
  ...
  if (!dataStorePaths.empty() &&
    ...
    for (auto file : files) {
      if (file.Right(4) == ".zip") {
        archives.push_front(
            libcomp::String("%1%2").Arg(dataStorePath).Arg(file).ToUtf8());
      }
    }
...
std::shared_ptr<Database> BaseServer::GetDatabase(
  ...
  bool configExists =
      configIter != configMap.end() && configIter->second != nullptr;
  ...
  switch (dbType) {
    case objects::ServerConfig::DatabaseType_t::SQLITE3:
      if (configExists) {
        auto sqlConfig =
            std::dynamic_pointer_cast<objects::DatabaseConfigSQLite3>(
                configIter->second);
        db = std::shared_ptr<libcomp::Database>(
            new libcomp::DatabaseSQLite3(sqlConfig));
      } else {
        LogServerCriticalMsg("No SQLite3 Database configuration specified.\n");
      }
      break;
    case objects::ServerConfig::DatabaseType_t::MARIADB:
      if (configExists) {
        auto sqlConfig =
            std::dynamic_pointer_cast<objects::DatabaseConfigMariaDB>(
                configIter->second);
        db = std::shared_ptr<libcomp::Database>(
            new libcomp::DatabaseMariaDB(sqlConfig));
      } else {
        LogServerCriticalMsg("No MariaDB Database configuration specified.\n");
      }
      break;
    default:
      LogServerCriticalMsg("Invalid database type specified.\n");
...
std::shared_ptr<Database> BaseServer::GetDatabase(
    const EnumMap<objects::ServerConfig::DatabaseType_t,
                  std::shared_ptr<objects::DatabaseConfig>>& configMap,
    bool performSetup, DataStore* pDataStore,
    ...
    if (!initFailure && createMockData) {
      ...
      if (configFile.IsEmpty()) {
        LogServerCriticalMsg(
            "Data mocking enabled but no setup file specified.\n");

        initFailure = true;
      } else {
        std::string configPath = GetConfigPath() + configFile.ToUtf8();
...
DataStore* BaseServer::GetDataStore() { return &mDataStore; }

TimerManager* BaseServer::GetTimerManager() { return &mTimerManager; }
...
void BaseServer::SetConfigPath(const std::string& path) { sConfigPath = path; }
...
bool BaseServer::ReadConfig(std::shared_ptr<objects::ServerConfig> config,
  ...
  if (nullptr == pObject || !config->Load(doc, *pObject)) {
    ...
    for (auto pair : config->GetLogLevels()) {
      ...
      if (to_underlying(libcomp::BaseLogComponent_t::Invalid) != comp) {
        ...
        LogServerCritical([&]() {
          return String(
                     "Unknown log component '%1'. Please check your "
                     "configuration.\n")
              .Arg(pair.first);
        });
      }
...
void BaseServer::CreateWorkers() {
  ...
  if (mConfig->GetMultithreadMode()) {
    ...
    switch (numberOfWorkers) {
      case 0:
        LogServerWarningMsg(
            "The maximum hardware concurrency level of this machine could not "
            "be detected. Multi-threaded processing will be disabled.\n");
  ...
  if (numberOfWorkers > 1) {
    LogServerDebug([numberOfWorkers]() {
      return String("Processing will be distributed between %1 workers.\n")
          .Arg(numberOfWorkers);
    });
  } else {
...
bool BaseServer::AssignMessageQueue(
    const std::shared_ptr<libcomp::EncryptedConnection>& connection) {
  std::shared_ptr<libcomp::Worker> worker =
      mWorkers.size() != 1 ? GetNextConnectionWorker() : mWorkers.front();

  if (!worker) {
    LogServerCriticalMsg(
        "The server failed to assign a worker to an incoming connection.\n");
...
std::shared_ptr<libcomp::Worker> BaseServer::GetNextConnectionWorker() {
  // By default return the least busy worker by checking shared_ptr message
  // queue use count
...
bool BaseServer::InsertDataFromFile(
    const libcomp::String& filePath, const std::shared_ptr<Database>& db,
...
bool BaseServer::LoadDataFromFile(const libcomp::String& filePath,
  ...
  const tinyxml2::XMLElement* objXml =
      doc.RootElement()->FirstChildElement("object");
  ...
  while (nullptr != objXml) {
    ...
    if (specificTypes.size() > 0 &&
        specificTypes.find(name) == specificTypes.end()) {
      continue;
    }

    auto typeHash =
        libcomp::PersistentObject::GetTypeHashByName(name, typeExists);
    if (!typeExists) {
      return false;
    }
    ...
    while (nullptr != memberXml) {
      if ("uid" == libcomp::String(memberXml->Attribute("name")).ToLower()) {
        ...
        if (uuid.IsNull()) {
          LogServerError([&]() {
            return String(
                       "Null UID specified for %1 entry while loading file "
                       "'%2'\n")
                .Arg(name)
                .Arg(filePath);
          });
...
BaseScriptEngine& BaseScriptEngine::Using<BaseServer>() {
  if (!BindingExists("BaseServer")) {
    ...
    binding.Func("GetDataStore",
                 &BaseServer::GetDataStore);  // Last call to binding
  }
