...
node new_node(void* key, void* value, color node_color, node left, node right) {
    node result = (rbtree_node)malloc(sizeof(struct rbtree_node_t));
    result->key = key;
    result->value = value;
...
node lookup_node(rbtree t, rbtree_key key, compare_func compare) {
    node n = t->root;
    while (n != NULL) {
...
rbtree_value rbtree_lookup(rbtree t, rbtree_key key, compare_func compare) {
...
void rbtree_insert(rbtree t, rbtree_key key, rbtree_value value, compare_func compare) {
...
void rbtree_delete(rbtree t, rbtree_key key, compare_func compare) {
    node child;
    node n = lookup_node(t, key, compare);
...
rbtree_value rbtree_take(rbtree t, rbtree_key key, compare_func compare) {
    node child;
    rbtree_value value;
    node n = lookup_node(t, key, compare);
    if (n == NULL) return rbtree_nil_value;  /* Key not found, do nothing */
    value = n->value;
    if (n->left != NULL && n->right != NULL) {
        /* Copy key/value from predecessor and then delete it instead */
        node pred = maximum_node(n->left);
